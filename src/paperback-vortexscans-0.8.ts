...
export const VortexScansInfo: SourceInfo = {
  ...
  websiteBaseURL: BASE,
  sourceTags: [{ text: 'Scanlation', type: TagType.GREY }],
  language: LanguageCode.ENGLISH,
}

export default class VortexScans extends Source {
  constructor(config: SourceConfig) {
    super(config)
  }

  // Utility to create request headers used by the site
  private buildRequest(url: string, method: 'GET' | 'POST' = 'GET'): Request {
    const headers = {
  ...
  async getSearchResults(query: SearchRequest, metadata: any): Promise<PagedResults> {
    ...
    $('.series-card, .post, .result-item').each((i, el) => {
      ...
      tiles.push(createMangaTile({
        id,
        title: createIconText({ text: decodeHTMLEntities(title) }),
  ...
  async getTags(): Promise<TagSection[]> {
    ...
    $('.genre-list a, .tags a').each((i, el) => {
      const t = $(el).text().trim()
      if (t) outTags.push({ id: t, label: t })
    })
